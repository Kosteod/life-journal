...
function DayTab({date, settings, saveSettings}) {
  ...
  // Объединяем одинаковые соседние блоки
  function getGrouped() {
    const groups = [];
    HOURS.forEach(h => {
              ...
              <div key={gi}>
                <div onClick={()=>{setEditHour(isEditing?null:g.startHour);if(g.hasBlock){setBlockType(g.type);setBlockNote(g.note||"");}}}
                  style={{display:"flex",alignItems:"center",gap:10,padding:"8px 10px",borderRadius:6,cursor:"pointer",
                    ...
                    opacity:isPast&&!g.hasBlock?0.3:1}}>
                  ...
                  {g.hasBlock&&(
                    <button onClick={e=>{e.stopPropagation();for(let h=g.startHour;h<g.endHour;h++) clearBlock(h);}} style={S.clearBtn}>✕</button>
                  )}
                </div>
                {isEditing&&(
